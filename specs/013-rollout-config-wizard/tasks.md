...
- [X] T039 [US1] Update `commands/config.md`'s Step 7 "Write" section so the bullet documenting the `local-config.yml` save states the literal, flat key `mcp_server` explicitly (e.g. `mcp_server: <selected server name/key>`), replacing the vague "name/key **only**" prose, with no alternate spelling or nesting (FR-018)
- [X] T040 [US1] Update `commands/brief-implement.md`'s Step 3.1 read step so it looks up the saved MCP server selection under the literal key `mcp_server` in `local-config.yml` explicitly, replacing the vague "saved name/key from `local-config.yml`" prose, with no alternate spelling or nesting (FR-018)
- [X] T041 [US1] Update `docs/foundation/vision.md` §6.2 and §8, both of which describe the `local-config.yml` MCP-server-selection save with vague "name/key" prose, to name the literal, flat key `mcp_server` explicitly (FR-018, FR-021)
- [X] T042 [US1] Update `docs/providers.md`'s "Adding a new provider" guide, which describes the same save with vague "name/key" prose, to name the literal, flat key `mcp_server` explicitly (FR-018, FR-021)
- [X] T043 [US1] Update `rollout-config.template.yml`'s header comment, which describes the same save with vague "name/key" prose, to name the literal, flat key `mcp_server` explicitly (FR-018)
...
**Purpose**: Close a gap found during PR review after implementation: FR-020 requires that `commands/brief-implement.md`'s MCP server resolution support a second path — running `commands/config.md`'s Step 2 zero/one/many-candidate detection heuristics fresh at runtime when the wizard was never run — in addition to the already-implemented first path (reading a saved selection from `local-config.yml`). Step 3.1's plan-only-mode degradation step currently ran unconditionally whenever there was no saved selection, with no fresh-detection fallback attempted first.

- [X] T044 [US1] Insert a fresh-detection fallback step into `commands/brief-implement.md`'s Step 3.1, positioned between point 2 ("Use exactly the MCP server the developer already registered and selected") and the plan-only-mode degradation step (renumbered to point 4 by this change): when `local-config.yml` has no saved `mcp_server` selection, perform the same MCP server discovery & candidate-resolution heuristics documented in `commands/config.md`'s Step 2 (introspection-based LaunchDarkly-capable signals, never a hardcoded or pinned server reference) fresh at runtime, then branch on candidate count exactly as follows: zero candidates — fall through to point 4's plan-only-mode degradation unchanged; exactly one candidate — use it for this run's MCP introspection and explicitly note in the agent's output that it was resolved via fresh detection rather than a saved `speckit.rollout.config` selection; more than one candidate — since `brief-implement.md` runs non-interactively as part of `/speckit.implement` and cannot pause for developer disambiguation the way `commands/config.md`'s Step 2 does, treat this the same as "no MCP available" and fall through to point 4's plan-only-mode degradation, same as the zero-candidates case (FR-020)

**Dependencies**: T044 depends on Phase 2 (Foundational) and the already-completed Phase 3 (US1)/Phase 10 content in `commands/brief-implement.md`; it must land after T040 (which already updated the same Step 3.1 section's saved-selection lookup) since both edit the same subsection.
